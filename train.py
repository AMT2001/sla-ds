...
# 设置训练的输入数据集路径
argparser.add_argument('--data_path', type=str, default='data', help='Path to the training dataset')
# 设置模型的放大倍数
argparser.add_argument('--upscale', type=str, default='x2', help='Scale factor for the input images, e.g. x2 for 2x super-resolution')
# 设置训练的batch大小
argparser.add_argument('--batch_size', type=int, default=32, help='Batch size for training')  
# 设置训练使用哪些变量作为输入，list类型
argparser.add_argument('--in_elements', nargs='+', default=['sla'], help='List of input elements for the model')
# 设置要输出的变量，list类型
...
# 设置训练的日志保存路径
argparser.add_argument('--run_path', type=str, default='run', help='Path to the log directory')
    ...
    if args.model_name == 'SRGAN':
        with tqdm(total=len(train_loader)) as pbar:
            for _, data in enumerate(train_loader, 0):
                ...
                out_put = model.train_generator(inputs, labels)
                model.train_discriminator(out_put, labels)
...
def validate_model(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader, 0):
            ...
            ssim += batch_ssim(outputs * 10, labels * 10)
...
if __name__ == "__main__":
    ...
    if args.model_name == 'DSDN':
        model_config['model_params']['data_path'] = args.data_path
    
    # 创建模型实例
    if model_config['model_name'] == 'SRGAN':
        model_config['model_params']['device'] = args.device
        model_config['model_params']['lr'] = args.learning_rate
    ...
    # 定义损失函数
    if args.model_name == 'DSDN':
        criterion = PixelStructureLoss(window_size=3, alpha=0.5, beta=0.8).to(device)
    else:
        criterion = MSELoss().to(device)
    # 开始训练
    
    # 创建模型保存路径
    save_path = os.path.join(args.run_path, f'model_{args.model_name}/{args.upscale}')
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    # 模型保存名称   
    in_str = '_'.join(args.in_elements)
    out_str = '_'.join(args.out_elements)
    save_name = f'{in_str}_{out_str}'
   
    # 创建损失保存路径
    val_path = os.path.join(args.run_path, f'val_{args.model_name}/{args.upscale}')
    ...
    for epoch in range(args.num_epochs):
        ...
        # 验证模型
        if model_config['model_name'] == 'SRGAN':
            val_loss, rmse, psnr, ssim = validate_model(model.generator, val_loader, criterion, device)
        else:
        ...
        if (epoch + 1) % (args.num_epochs // 10) == 0:
            if model_config['model_name'] != 'SRGAN':
                ...
                model.save_model(os.path.join(save_path, f'{save_name}_{epoch+1}.pth'))
        ...
        if (epoch + 1) % min(args.lr_decay_step, args.num_epochs // 10) == 0:
            if model_config['model_name'] != 'SRGAN':
                ...
                for param_group in model.g_optim.param_groups:
                    param_group['lr'] *= args.lr_decay  
                for param_group in model.d_optim.param_groups:
                    param_group['lr'] *= args.lr_decay  
