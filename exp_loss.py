...
from loss.maeLoss import MAELoss
from loss.mseLoss import MSELoss
from loss.contentLoss import ContentLoss
...
# 设置训练的日志保存路径
argparser.add_argument('--run_path', type=str, default='exp_loss', help='Path to the log directory')
# 设置损失函数
argparser.add_argument('--loss_name', type=str, default='MAE', help='Loss function to use, e.g. MSE, MAE, ContentLoss, PixelStructureLoss')
# 解析输入参数
...
if __name__ == "__main__":
    ...
    # 模型保存名称   
    save_name = f'{args.loss_name}'
    # 创建损失保存路径
    val_path = os.path.join(args.run_path, f'val_{args.model_name}_{args.loss_name}')
